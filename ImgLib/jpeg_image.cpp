    ...
    bool SaveJPEG(const Path& file, const Image& image) {
        ...
        */
        ...
        while (cinfo.next_scanline < cinfo.image_height) {
            ...
            std::vector<JSAMPLE> pixels;

            for (int j = 0; j < image.GetWidth(); ++j) {
                const auto pixel = image.GetPixel(j, cinfo.next_scanline);
                pixels.push_back(static_cast<JSAMPLE>(pixel.r));
                pixels.push_back(static_cast<JSAMPLE>(pixel.g));
                pixels.push_back(static_cast<JSAMPLE>(pixel.b));
            }

            row_pointer[0] = pixels.data();
